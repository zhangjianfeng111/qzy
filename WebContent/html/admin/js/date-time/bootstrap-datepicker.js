!function(d) {
    ...
    var a = function(h, g) {
        var i = this;
        this.element = d(h);
        this.language = g.language || this.element.data("date-language") || "zh";
        this.language = this.language in e ? this.language : this.language.split("-")[0];
        this.language = this.language in e ? this.language : "zh";
        ...
        fill: function() {
            ...
            while (B.valueOf() < g) {
                if (B.getUTCDay() == this.weekStart) {
                    o.push("<tr>");
                    if (this.calendarWeeks) {
                        var h = new Date(+B + (this.weekStart - B.getUTCDay() - 7) % 7 * 864e5), k = new Date(+h + (7 + 4 - h.getUTCDay()) % 7 * 864e5), j = new Date(+(j = f(k.getUTCFullYear(), 0, 1)) + (7 + 4 - j.getUTCDay()) % 7 * 864e5), q = (k - j) / 864e5 / 7 + 1;

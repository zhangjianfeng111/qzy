<#escape x as x!"">
   ...
   <!--
   	...
   	<#if (start_i-1) gt 0>
			<li class="prev"> <a  href='javascript:ajaxPage("${ajaxPageShowId}","${pageUrl?replace(replaceStr, (start_i-1)?string)}")'>上一页</a></li>
   	</#if>
   
	<#list start_i ..end_i as i>	
		<#if i == currentIndex>
			<li class="active"><a href='javascript:ajaxPage("${ajaxPageShowId}","${pageUrl?replace(replaceStr, i?string)}")'>${i}</a></li>
			<#else>
			<li><a  href='javascript:ajaxPage("${ajaxPageShowId}","${pageUrl?replace(replaceStr,i?string)}")'>${i}</a></li>
		</#if>
	</#list>
	
 	<#if (end_i+1) lt total >
		<li><a href='javascript:ajaxPage("${ajaxPageShowId}","${pageUrl?replace(replaceStr, (end_i+1)?string)}")'>下一页</a></li>
   	</#if> 
